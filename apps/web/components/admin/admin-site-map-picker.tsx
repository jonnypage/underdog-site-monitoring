...
const DEFAULT_CENTER: google.maps.LatLngLiteral = { lat: 18.081, lng: -88.564872 };

const mapContainerStyle = { width: "100%", height: "280px", borderRadius: "0.375rem" };
...
function parseCoord(latStr: string, lngStr: string): google.maps.LatLngLiteral | null {
...
export function AdminSiteMapPicker({ latStr, lngStr, onLatLngChange }: AdminSiteMapPickerProps) {
  ...
  if (!apiKey) {
    return (
      <div className="rounded-md border border-dashed border-border bg-muted/30 p-4 text-sm text-muted-foreground">
        Set <code className="rounded bg-muted px-1 py-0.5 text-xs">NEXT_PUBLIC_GOOGLE_MAPS_EMBED_API_KEY</code> in{" "}
        <code className="rounded bg-muted px-1 py-0.5 text-xs">.env.local</code> and enable the{" "}
        <strong className="font-medium text-foreground">Maps JavaScript API</strong> (and Embed API for dashboard iframes) on that key.
      </div>
    );
  }

  if (loadError) {
    return <p className="text-sm text-red-600">Could not load Google Maps. Check the API key and billing.</p>;
  }
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        Use the <strong className="font-medium text-foreground">Map / Satellite</strong> control (top of the map) to switch views; hybrid shows satellite imagery with labels.{" "}
        {position
