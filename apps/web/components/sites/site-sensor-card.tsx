...
export function SiteSensorCard({
  siteId,
  sensor,
  chartData,
}: SiteSensorCardProps) {
  return (
    <Link
      href={`/sites/${siteId}/sensors/${sensor.key}`}
      className='block group'
    >
      <Card className='transition-all hover:border-primary/50 hover:shadow-md cursor-pointer h-full overflow-hidden'>
          ...
          <div className='flex items-start justify-between gap-2'>
            <div className='flex items-start gap-2.5'>
              ...
              <div className='space-y-0.5'>
                <CardTitle className='flex h-6 text-sm items-center leading-tight group-hover:text-primary transition-colors'>
                  {sensor.displayName}
                </CardTitle>
              </div>
            </div>
            <div className='text-right'>
              <p className='text-2xl font-bold leading-none flex items-top gap-1'>
                {sensor.currentValue != null
                  ? `${Number(sensor.currentValue).toFixed(2)}`
                  : '-'}

                <span className='text-xs font-normal'>
                  {sensor.currentValue != null ? `${sensor.unit}` : ''}
                </span>
              </p>
            </div>
          </div>
